...
class TrainProgram
  ...
  def show_trains_carriage_list(train)
    ...
    train.carriage_list.each do |carriage|
      ...
      if carriage.type == :cargo
        puts "Занято: #{carriage.occupied_amount} объема"
        puts "Свободно: #{carriage.empty_amount} объема"
      elsif carriage.type == :passenger
        puts "Занято: #{carriage.occupied_places} мест"
        puts "Свободно: #{carriage.empty_places} мест"
        carriage_number += 1
      end
    end
  end
  ...
  def show_trains_on_station(station)
    station.train_list do |train|
      puts "Номер поезда: #{train.train_number}, типа поезда: #{train.type}, количество вагонов: #{train.carriage_list.length}"
    end
